...
function clearAllCards() {
  ...
  cards.innerHTML = ""; /*clear all child from object cards*/
}
...
function cardImg(url) {
  ...
  div.style = `background-image: url(${url})`; /*use image from web url*/
...
function cardBody(name, date, uri, wikipediaUrl) {
  ...
  const dateAnchor = createAnchor(uri, date.toLocaleDateString()); //display time with style of LocaleDate String dd/mm/yyyy
...
function buildCardForObservation(observation) {
  const {
    ...
    isIntroduced,
    isThreatened,
  } = observation; //get properties from observation and put into diff named objects
...
function toggleLoading(isLoading) {
  if (isLoading) {
    ...
    document.getElementById("submit").disabled = true; //during searching disable submit bottom
  } else {
    var icon = document.getElementById("icon");
    icon.innerHTML = "";
    ...
    document.getElementById("submit").disabled = false; //when false, let user have chance to put in other text
  }
